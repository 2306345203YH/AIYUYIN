...
from module.mel_processing import spectrogram_torch  # 添加频谱处理依赖
...
def load_models(gpt_path, sovits_path, bert_path, cnhubert_path):
    ...
    hps = dict_s2["config"]  # 保存hps用于频谱计算
...
def get_spepc(hps, audio_data, sampling_rate):
    """将音频转换为频谱图"""
    audio = torch.FloatTensor(audio_data)
    audio_norm = audio.unsqueeze(0).to(device)
    if is_half:
        audio_norm = audio_norm.half()
...
def text_to_speech(ref_wav_path, prompt_text, text, prompt_language="zh", text_language="zh"):
    ...
    # 生成参考频谱
    audio_data, sr = librosa.load(ref_wav_path, sr=hps["data"]["sampling_rate"])
    refer = get_spepc(hps, audio_data, sr).to(device)
    if is_half:
        refer = refer.half()
