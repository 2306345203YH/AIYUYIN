...
# is_half = torch.cuda.is_available()  # 可改为 False 测试杂音
is_half = False  # 可改为 False 测试杂音
hps = None
t2s_model = None
vq_model = None
bert_model = None
...
def text_to_speech(ref_wav_path, prompt_text, text, prompt_language="zh", text_language="zh"):
    ...
    # 生成语义特征（移除 prompt_semantic，确保仅基于目标文本）
    with torch.no_grad():
        pred_semantic, _ = t2s_model.model.infer_panel(
            all_phoneme_ids,
            all_phoneme_len,
            None,  # 移除 prompt_semantic，仅依赖目标音素和BERT
            bert,
            top_k=15,
            top_p=1.0,
            temperature=1.0
    ...
    #     audio /= max_audio
    # 改进音频归一化
    audio = audio / torch.max(torch.abs(audio)) * 0.9  # 限制幅度到 0.9，避免削波
    print(f"Audio max: {audio.max()}, min: {audio.min()}")
