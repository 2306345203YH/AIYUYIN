...
def text_to_speech(ref_wav_path, prompt_text, text, prompt_language="zh", text_language="zh"):
    ...
    # 处理爆音和结尾噪声
    zero_wav = torch.zeros(int(sr * 0.00001), device=device)  # 0.2 秒静音
    processed_audio = []
    for i, batch in enumerate(audio):
        batch_audio = []
        for j, audio_fragment in enumerate(batch):
            max_audio = torch.abs(audio_fragment).max()  # 防止爆音
            if max_audio > 1:
                audio_fragment /= max_audio
                print(f"Detected potential clipping, normalized fragment {i},{j} with max: {max_audio}")
            audio_fragment = torch.cat([audio_fragment, zero_wav], dim=0)  # 添加静音结尾
            batch_audio.append(audio_fragment)
        processed_audio.append(batch_audio)

    # 将处理后的音频转为张量
    audio = torch.stack([torch.stack(batch) for batch in processed_audio])
    audio_np = audio.cpu().numpy()[0][0]  # 假设单通道输出
    ...
    # 应用带通滤波器（500 Hz - 6000 Hz）
    audio_np = bandpass_filter(audio_np, lowcut=500, highcut=4000, fs=hps["data"]["sampling_rate"], order=6)
