...
from scipy.signal import butter, lfilter  # 添加滤波器依赖

# 全局变量
device = "cuda" if torch.cuda.is_available() else "cpu"
is_half = False  # 已禁用半精度
hps = None
t2s_model = None
vq_model = None
bert_model = None
...
def text_to_speech(ref_wav_path, prompt_text, text, prompt_language="zh", text_language="zh"):
    ...
    with torch.no_grad():
        pred_semantic, _ = t2s_model.model.infer_panel(
            ...
            top_k=20,  # 增加 top_k
            top_p=1.0,
            temperature=0.8  # 降低 temperature
        )
    ...
    audio_raw = audio.cpu().numpy()
    soundfile.write("output_raw.wav", audio_raw * 32767, sr, subtype='PCM_16')
    ...
    # 应用低通滤波器去噪（截止频率 8000 Hz）
    audio_np = lowpass_filter(audio_np, cutoff=4000, fs=hps["data"]["sampling_rate"])
...
if __name__ == "__main__":
    ...
    ref_wav_path = "output4/changli_004.wav"
    prompt_text = "而我只是一个有幸接过游丝的引路人,是助你串联起过去与未来的契机。"
    text = "大家好，今天天气很好。"
