...
## 2026-09-14 MMD 舞台 M1

- 已安装前端依赖 `three`、`three-stdlib` 和对应 TypeScript 类型。
- 新增 `web/src/MMDStage.tsx`，支持 PMX 加载、模型缩放、待机呼吸、眨眼和语音播放时基础口型。
- 角色表新增 `mmd_model` 字段并完成旧数据库自动迁移；艾雅法拉默认绑定 `yanhe_kabashiki`。
- 设置页增加舞台模型选择和全局舞台开关；Three.js 舞台使用动态加载，未使用时不加载大资源。
- 新增 `/api/mmd/motions` 动作清单接口，为下一阶段 VMD 动作接入保留入口。
- `app/web_app.py` 语法检查、前端生产构建、健康接口、模型清单和 PMX 静态文件访问验证通过。
- 现阶段尚未加入 VMD 手势动作和逐词口型，下一阶段接入动作库与情绪映射。
