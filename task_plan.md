...
## MMD 角色舞台第一版（2026-09-14）

目标：把 `椛暗式-言和ver1.0` 接入网页聊天，在回复播放时提供角色模型和基础动作响应。

- [completed] 33. 安装 Three.js MMD 渲染依赖并接入按需加载
- [completed] 34. 增加 PMX 模型扫描、静态资源和角色绑定字段
- [completed] 35. 实现舞台开关、待机呼吸、眨眼和基础口型
- [completed] 36. 构建、接口、模型文件访问和数据库迁移验证
- [pending] 37. 接入 VMD 手势动作、情绪映射和动作平滑过渡
- [pending] 38. 增加逐词口型对齐和动作映射编辑
