...
@register_method("cut_safe")
def cut_safe(inp, target_len=30, max_len=38):
    """Split mainly at sentence endings and avoid artificial comma pauses."""
    inp = inp.strip("\n")
    ...
    def append_chunk(value):
        value = value.strip()
        if not value or set(value).issubset(punctuation):
            return
        if value[-1] not in "。！？!?…":
            value = value.rstrip("，,；;：:、") + "。"
        chunks.append(value)
    ...
    for sentence in sentences:
        if len(sentence) > max_len:
            if current:
                ...
                current = ""
            weak_parts = re.split(r"(?<=[，,；;：:、])", sentence)
            weak_current = ""
            for part in weak_parts:
                while len(part) > max_len:
                    append_chunk(part[:max_len])
                    part = part[max_len:]
                if weak_current and len(weak_current) + len(part) > max_len:
                    append_chunk(weak_current)
                    weak_current = part
                else:
                    weak_current += part
                if len(weak_current) >= target_len:
                    append_chunk(weak_current)
                    weak_current = ""
            if weak_current:
                append_chunk(weak_current)
