...
PROJECT_ROOT = Path(
    os.environ.get("AIYUYIN_PROJECT_ROOT", Path(__file__).resolve().parents[3])
)

# config/gpt_sovits/tts_infer.yaml
"""
custom:
  ...
  device: cpu
  is_half: false
  t2s_weights_path: models/voices/changli/changli.ckpt
  vits_weights_path: models/voices/changli/changli.pth
  version: v2
default:
  bert_base_path: models/gpt_sovits/chinese-roberta-wwm-ext-large
  cnhuhbert_base_path: models/gpt_sovits/chinese-hubert-base
...
class TTS_Config:
    default_configs={
        "default":{
                ...
                "is_half": False,
                "version": "v1",
                "t2s_weights_path": str(PROJECT_ROOT / "models/gpt_sovits/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt"),
                "vits_weights_path": str(PROJECT_ROOT / "models/gpt_sovits/s2G488k.pth"),
                "cnhuhbert_base_path": str(PROJECT_ROOT / "models/gpt_sovits/chinese-hubert-base"),
                "bert_base_path": str(PROJECT_ROOT / "models/gpt_sovits/chinese-roberta-wwm-ext-large"),
            },
        "default_v2":{
                "device": "cpu",
                "is_half": False,
                "version": "v2",
                "t2s_weights_path": str(PROJECT_ROOT / "models/gpt_sovits/gsv-v2final-pretrained/s1bert25hz-5kh-longer-epoch=12-step=369668.ckpt"),
                "vits_weights_path": str(PROJECT_ROOT / "models/gpt_sovits/gsv-v2final-pretrained/s2G2333k.pth"),
                "cnhuhbert_base_path": str(PROJECT_ROOT / "models/gpt_sovits/chinese-hubert-base"),
                "bert_base_path": str(PROJECT_ROOT / "models/gpt_sovits/chinese-roberta-wwm-ext-large"),
            },
    }
    ...
    def __init__(self, configs: Union[dict, str]=None):
        ...
        # 设置默认配置文件路径
        configs_base_path = str(PROJECT_ROOT / "config")
        os.makedirs(configs_base_path, exist_ok=True)
        self.configs_path = os.path.join(configs_base_path, "tts_runtime.yaml")
        ...
        self.t2s_weights_path = self._resolve_project_path(self.t2s_weights_path)
        self.vits_weights_path = self._resolve_project_path(self.vits_weights_path)
        self.bert_base_path = self._resolve_project_path(self.bert_base_path)
        self.cnhuhbert_base_path = self._resolve_project_path(self.cnhuhbert_base_path)
...
class TTS:
    ...
    def run(self, inputs:dict):
            ...
            inputs (dict):
                {
                    ...
                    "serial_vits": False,         # bool. decode each fragment separately to avoid boundary clipping.
        ...
        serial_vits = inputs.get("serial_vits", False)
        ...
        try:
            ...
            for item in data:
                ...
                if speed_factor == 1.0 and not serial_vits:
                    # ## vits并行推理 method 2
                    pred_semantic_list = [item[-idx:] for item, idx in zip(pred_semantic_list, idx_list)]
